...
WXPay.mix('WXCallBack', function(fn)
{
	return function(req, res, next)
	{
		...
		util.pipe(req, function(err, data)
		{
			...
			util.parseXML(xml, function(err, msg)
			{
				...
				var total_fee = parseInt(msg.total_fee)/100;
				//控制访问
				redisClient.incr(key,function( err, id ) 
				{
					...
					{
	                    //写一下充值记录
	                    log.set('beforeCash', user.get('cash'));

	                   	user.increment('cash', total_fee * 100);
					...
					{
						console.log(error);
						console.log('余额充值出错'+"tradeNo:"+msg.out_trade_no);
						//req.wxmessage = msg;
			...
			util.parseXML(xml, function(err, msg)
			{
				...
				redisClient.incr(key,function( err, id ) 
				{
					...
					}).then(function(user)
					{
	                    ...
	                    if((goldNum && goldNum >0) || (Diamond && Diamond > 0))
	                    {
	                    	...
	                    	user.increment('goldMax', parseInt(goldNum));
							//实际支付超过500 额外赠送88
	                    	...
	                    	user.increment('goldNum',parseInt(goldNum*3 + Diamond * 100));
	                    }
	                    else
	                    {
	                    	if(type == 1)//充值金币
	                    	{
	                    		...
	                    		user.increment('goldMax', parseInt(goldMax));
	                    		user.increment('Diamond', Math.floor(total_fee/500) * 88);
	                    		user.increment('goldNum',parseInt(goldNum));
	                    	}
	                    	else
	                    	{
	                    		Diamond = total_fee;
