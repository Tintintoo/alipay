		...
		util.pipe(req, function(err, data)
		{
			...
			util.parseXML(xml, function(err, msg)
			{
				...
				query.first().then(function(data)
				{
					...
					//if(fee > msg.total_fee)//如果两边价格不一致
					//{
					//	data.set('OrderError', '支付结果与实际价钱不一致,需要做处理,支付金额为'+msg.total_fee+"实际需要支付"+fee);
					//	data.save();
					//	return AV.Promise.error('There was an error.');
					//}
					data.increment('orderState', 1);
					data.set('realCharge', msg.total_fee);
					data.save();
				...
				}).then(function(data)
				{
                    ...
                    if (vipType > 0 && msg.total_fee < 300)//购买在300以下 
                    {
					...
					data.increment('goldMax', parseInt(gold));
					//实际支付超过500 额外赠送88
                    data.increment('Diamond', parseInt(Diamond) + Math.floor(msg.total_fee/500) * 88);
                    console.log("Diamond:"+parseInt(Diamond)+"+"+Math.floor(msg.total_fee/500) * 88);
                    data.increment('goldNum',parseInt(gold + Diamond * 100));
                    data.increment('BonusPoint', parseInt(msg.total_fee));
                    console.log('userid:'+ data.get('userID')+ 'oldGold:' + data.get('goldNum')+"goldMax:"+gold+"Diamond:"+Diamond);

                    //记录订单号
                    log.set('tradeNo', msg.out_trade_no);
                    //实际支付金额
                    log.set('money', msg.total_fee);
                      //记录用户id
                    log.set('userID', data.get('userID'));
                      //记录购买物品
                    log.set('goldMax', gold);
                    log.set('goldNum', gold + Diamond*100);
